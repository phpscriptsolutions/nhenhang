...
class XUploadAction extends CAction
{
	...
	public function init()
	{
		...
		$this->path = Yii::app()->params['storage']['baseStorage'];
        $this->_subfolder = "tmp";

		if(!is_dir($this->path)){
			throw new CHttpException(500, "{$this->path} does not exists.");
		}else if(!is_writable($this->path.DS.$this->_subfolder)){
		...
		$this->alowType = explode(",", $this->alowType);
	...
	public function run()
	{
		...
		if(empty($model->file)){
			echo json_encode(array("error"=>"1","msg"=>Yii::t('admin','Không hỗ trợ định dạng') )); exit;
		}
		$model->mime_type = $model->file->getType();
		$model->size = $model->file->getSize();
		$model->name = date("YmdHis_"). $model->file->getName();

		//echo json_encode(array("error"=>"1","msg"=>Yii::t('admin','Dung lượng file vượt quá {size}',array('{size}'=>$this->maxSize)) )); exit;
        $tempPath = $model->file->getTempName();
        $fileMime = function_exists('mime_content_type')?mime_content_type($tempPath):$model->file->getType();
        $isShellFile = false;
		...
		if(count($this->alowType) > 0 && !in_array($model->mime_type, $this->alowType)){
			echo json_encode(array("error"=>"1","msg"=>Yii::t('admin','Bạn chỉ được chọn file định dạng {type}',array('{type}'=>implode(',',$this->alowType))) )); exit;
		}

        if(!in_array($fileMime, $this->alowType) || $isShellFile){
            echo json_encode(array("error"=>"1","msg"=>Yii::t('admin','Không hỗ trợ định dạng file này!') )); exit;
        }

		$fileSize = $model->size /(1024*1024);
		$maxSize = str_replace("M", "", $this->maxSize);
		
		//if(isset($this->maxSize) && ($model->size > $this->maxSize)){
		if(isset($this->maxSize) && ($fileSize > $maxSize)){
			echo json_encode(array("error"=>"1","msg"=>Yii::t('admin','Dung lượng file '.$model->size .'vượt quá {size} MB',array('{size}'=>$this->maxSize)) )); exit;
		}
		
		
		if ($model->validate()) {
			$path = $this->path."/".$this->_subfolder."/";
			//$path = Yii::app()->params['storage']['baseStorage'].'tmp'.DS;
			if(!is_dir($path)){
				mkdir($path);
			}
			...
			echo CVarDumper::dumpAsString($model->getErrors()); exit();
