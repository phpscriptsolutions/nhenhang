...
class ImageCrop
{
	...
	function loadImage()
	{
		...
		if(empty($image_size)){
			$this->_debug('loadImage', "That file cannot be loaded.");
			return false;
		}
		$mine = $image_size["mime"];
		$mine = str_replace("image/", "", $mine);
		$func = "imagecreatefrom$mine";
        ...
        $newFileJpg = Yii::app()->getRuntimePath()."/".time().".jpg";
	...
	function cropImage($desFile, $desWidth, $desHeight, $quality)
	{
        ...
        $log = new KLogger('upload_avatar', KLogger::INFO);
        $log->LogInfo('$this->_imgFinal:'.$this->_imgFinal.'|$this->_imgOrig:'.$this->_imgOrig.'|$this->_cropX:'.$this->_cropX.'|$this->_cropY:'.$this->_cropY.'|$this->_cropWidth:'.$this->_cropWidth.'|$this->_cropHeight:'.$this->_cropHeight, false);
