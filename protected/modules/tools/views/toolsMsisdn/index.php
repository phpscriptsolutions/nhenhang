...
$this->pageLabel = Yii::t("admin","Danh sách ToolsMsisdn");
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('admin-tools-msisdn-model-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<?php if($numPage){
	for($i=1; $i<=$numPage; $i++){
		$linkExport = Yii::app()->createUrl('/tools/toolsMsisdn/export', array('id'=>$id, 'page'=>$i));
		echo "<a href='$linkExport'>Export-$i</a>&nbsp;&nbsp;&nbsp;";
	}
...
if(Yii::app()->user->hasFlash('ToolsMsisdn')){
    echo '<div class="flash-success">'.Yii::app()->user->getFlash('ToolsMsisdn').'</div>';
}


$this->widget('application.widgets.admin.grid.CGridView', array(
	'id'=>'admin-tools-msisdn-model-grid',
	...
	'columns'=>array(
		...
		array(
            ...
            'header'=>CHtml::dropDownList('pageSize',$pageSize,array(10=>10,30=>30,50=>50,100=>100),array(
                                                                                  'onchange'=>"$.fn.yiiGridView.update('admin-tools-msisdn-model-grid',{ data:{pageSize: $(this).val() }})",
                                                                                )),
