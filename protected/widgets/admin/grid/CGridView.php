...
Yii::import('application.widgets.admin.grid.CBaseListView');
...
class CGridView extends CBaseListView
{
	...
	/**
	 * create data array for export to excel
	 */
	protected function getDataArrayExport()
	{
		//$this->dataProvider->pagination->pageSize=100;
		$total=$this->dataProvider->getTotalItemCount();
		$data2=$this->columns;
		foreach ($data2 as $key =>$value){
			$exportData[] = array($value->id, isset($value->name)?$value->name:'');
		}
		...
		$dataArrayExport[] = $exportData2;
		//$this->dataProvider->setPagination(65555);
		//$this->dataProvider->pagination->pageSize=1000;
		Yii::app()->user->setState('pageSize',1000);
		$data=$this->dataProvider->getData(true);
		foreach ($data as $value){
			$val = array();
			foreach ($exportData2 as $cl){
				$val[] = $value->$cl;
			}
			$dataArrayExport[] = $val;
		}
	...
	public function renderItems()
	{
		...
		{
			//$this->widget('application.widgets.admin.ExportGridviewExcel', array('_data'=>$this->getDataArrayExport()));
