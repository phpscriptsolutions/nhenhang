...
abstract class CBaseListView extends CWidget
{
	...
	public function renderSummary()
	{
		...
		echo '<div class="'.$this->summaryCssClass.'">';
		echo '<div style="float: left">'.$this->titleText.'</div>';
		if($this->enablePagination)
		{
			...
			echo '<div style="float: right">'.strtr($summaryText,array(
				'{start}'=>$start,
				'{end}'=>$end,
				'{count}'=>$total,
		...
		{
			if(($summaryText=$this->summaryText)===null)
				$summaryText=Yii::t('zii','Total {count} result(s).', $count);
			echo '<div style="float: right">'.strtr($summaryText,array(
				'{count}'=>$count,
				'{start}'=>1,
				'{end}'=>$count,
	...
	public function renderPager()
	{
		...
		if($pager['pages']->getPageCount()>1)
		{
			...
			//$this->widget('application.widgets.admin.pagers.CListPager',$pager);
			echo '</div>';
		}
		else
			$this->widget($class,$pager);
