...
$this->pageLabel = Yii::t("admin","Danh sách Html");
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('admin-html-model-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
if(Yii::app()->user->hasFlash('Html')){
    echo '<div class="flash-success">'.Yii::app()->user->getFlash('Html').'</div>';
}
...
$this->widget('zii.widgets.grid.CGridView', array(
	...
	'columns'=>array(
		...
		array(
			'name'=>'title',
			'value'=>'Chtml::link($data->title,Yii::app()->createUrl("html/update",array("id"=>$data->id)))',
            'type'=>'raw',
		),
		'url_key',
		//'content',
		...
		*/
		array(
            ...
            'header'=>CHtml::dropDownList('pageSize',$pageSize,array(10=>10,30=>30,50=>50,100=>100),array(
                                                                                  'onchange'=>"$.fn.yiiGridView.update('admin-html-model-grid',{ data:{pageSize: $(this).val() }})",
                                                                                )),
