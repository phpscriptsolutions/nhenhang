	...
	*/
	...
	class KLogger
	{
		...
		public function __construct( $filepath , $priority )
		{
			...
			$path = Yii::app()->getRuntimePath().DS.$filepath.".log";
			if(file_exists($path) && filesize($path) > 500*1024*1024){
				rename($path,Yii::app()->getRuntimePath().DS.$filepath."_".date('YmdHis').".log");
			}
		...
		public function LogInfoNoEcho($line)
		{
			$this->Log( $line , KLogger::INFO );			
		}
